...
# FIX AND PAD THE CODING BED FILE
	...
	# remove annotation fields
		...
		awk 1 ${CODING_BED} \
			...
			| sed 's/^chr//g' \
			| egrep "^[0-9]|^X|^Y" \
		>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}_${CODING_BED_NAME}-${CODING_MD5}-${PADDING_LENGTH}-BP-PAD.bed
...
# FIX AND PAD THE TARGET BED FILE
	...
	# THIS IS FOR SLICING
		...
		awk 1 ${TARGET_BED} \
			...
			| sed 's/^chr//g' \
			| egrep "^[0-9]|^X|^Y" \
		>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}_${TARGET_BED_NAME}-${PADDING_LENGTH}-BP-PAD.bed
	...
	# FOR DATA PROCESSING AND METRICS REPORTS AS WELL.
		...
		# filter to karyotypic chromosomes
			...
			awk 1 ${CODING_BED} \
				...
				| sed 's/^chr//g' \
				| egrep "^[0-9]|^X|^Y" \
			>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${CODING_BED_NAME}-${CODING_MD5}.bed
...
# FIX AND PAD THE ANNOTATED CODING BED FILE
	...
	# filter to karyotypic chromosomes
		...
		awk 1 ${CODING_BED} \
			...
			>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}_${CODING_BED_NAME}-${CODING_MD5}-${PADDING_LENGTH}-BP-PAD-ANNOTATED.bed
	...
	# FOR DATA PROCESSING AND METRICS REPORTS AS WELL.
		...
		# filter to karyotypic chromosomes
			...
			awk 1 ${BAIT_BED} \
				...
				| sed 's/^chr//g' \
				| egrep "^[0-9]|^X|^Y" \
			>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${BAIT_BED_NAME}.bed
...
# FIX THE TITV BED FILE FOR DATA PROCESSING AND METRICS REPORTS.
	...
	# filter to karyotypic chromosomes
		...
		awk 1 ${TITV_BED} \
			...
			| sed 's/^chr//g' \
			| egrep "^[0-9]|^X|^Y" \
		>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${TITV_BED_NAME}.bed
...
# THIS IS PADDED WITH 250 BP AND THEN MERGED FOR OVERLAPPING REGIONS.
	...
	cat ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${CODING_BED_NAME}-${CODING_MD5}.bed \
	${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${BAIT_BED_NAME}.bed \
		| sort \
			-k 1,1 \
			-k 2,2n \
		...
		| singularity exec ${ALIGNMENT_CONTAINER} bedtools \
			merge \
			-i - \
	>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${BAIT_BED_NAME}-${CODING_BED_NAME}-${CODING_MD5}-${GVCF_PAD}-BP-PAD-GVCF.bed
...
# print CHROMOSOME, START, END, ARM (TAB DELIMITED) TO MAKE A BED FILE.
	...
	sed 's/^chr//g' ${CYTOBAND_BED} \
		...
		| awk 'BEGIN {OFS="\t"} \
			{print $1,$3,$4,$2}' \
	>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}.CHROM_ARM.bed
...
# ti/tv bed is used as the target since it shouldn't change
	...
	# bait bed
		...
		(grep "^@SQ" ${REF_DICT} \
			...
			${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${BAIT_BED_NAME}.bed) \
		>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${BAIT_BED_NAME}-picard.bed
	...
	# target-TITV bed
		...
		(grep "^@SQ" ${REF_DICT} \
			; awk 'BEGIN {OFS="\t"} \
				{print $1,($2+1),$3,"+",$1"_"($2+1)"_"$3}' \
			${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${TITV_BED_NAME}.bed) \
		>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${TITV_BED_NAME}-picard.bed
	...
	# add a header
		...
		awk 1 ${CODING_BED} \
			...
			| awk 'BEGIN {print "chromosome" "\t" "start" "\t" "end" "\t" "name"}  \
				{print $0}' \
		>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${CODING_BED_NAME}-${CODING_MD5}.exomeDepth.input.bed
	...
	# merge overlapping intervals
		...
		awk 1 ${CODING_BED} \
			...
			| singularity exec ${ALIGNMENT_CONTAINER} bedtools \
				merge \
				-i - \
		>| ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${SM_TAG}-${CODING_BED_NAME}-${CODING_MD5}.exomeDepth.input.merged.bed
