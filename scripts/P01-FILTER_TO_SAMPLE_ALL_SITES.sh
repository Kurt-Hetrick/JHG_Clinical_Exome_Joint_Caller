...
# Filter to all sites for the sample

START_FILTER_TO_SAMPLE_ALL_SITES=`date '+%s'`

	# construct command line

		CMD="singularity exec ${ALIGNMENT_CONTAINER} java"
		...
		CMD=${CMD}" SelectVariants"
			...
			CMD=${CMD}" --variant ${CORE_PATH}/${OUTPUT_DIR}/${FAMILY}/VCF/${FAMILY}.VQSR.ANNOTATED.ALL.SITES.vcf.gz"
			CMD=${CMD}" --sample-name ${SM_TAG}"
		CMD=${CMD}" --output ${CORE_PATH}/${OUTPUT_DIR}/${FAMILY}/${SM_TAG}/VCF/FILTERED_ON_BAIT/${SM_TAG}.ALL_SITES.vcf.gz"
...
# write out timing metrics to file
	...
	echo ${SM_TAG}_${OUTPUT_DIR},P01,FILTER_TO_SAMPLE_ALL_SITES,${HOSTNAME},${START_FILTER_TO_SAMPLE_ALL_SITES},${END_FILTER_TO_SAMPLE_ALL_SITES} \
