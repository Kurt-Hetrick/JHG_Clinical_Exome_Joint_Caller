...
START_FILTER_FAMILY_TO_BAIT_PLUS_PAD=`date '+%s'`
	...
	# construct command line
		...
		CMD=${CMD}" intersect"
			...
			CMD=${CMD}" -b ${CORE_PATH}/${OUTPUT_DIR}/TEMP/${FAMILY}_${CODING_BED_NAME}-${CODING_MD5}-${PADDING_LENGTH}-BP-PAD.bed"
		CMD=${CMD}" | singularity exec ${ALIGNMENT_CONTAINER} bgzip"
			CMD=${CMD}" -@ ${THREADS}"
			CMD=${CMD}" -c"
		# THIS OUTPUT MIGHT END UP IN TEMP
		CMD=${CMD}" >| ${CORE_PATH}/${OUTPUT_DIR}/${FAMILY}/VCF/${FAMILY}.VQSR.ANNOTATED.ALL.SITES.vcf.gz"
		CMD=${CMD}" &&"
...
# write out timing metrics to file
	...
	echo ${FAMILY}_${OUTPUT_DIR},N01,FILTER_FAMILY_TO_BAIT_PLUS_PAD,${HOSTNAME},${START_FILTER_FAMILY_TO_BAIT_PLUS_PAD},${END_FILTER_FAMILY_TO_BAIT_PLUS_PAD} \
